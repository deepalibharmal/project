<?php

error_reporting(0);
include("connect.php");
if (isset($_POST['submitted']))
{

$destn=$_POST['destn'];
$source=$_POST['source'];

$sql="SELECT `trainname`,`trainid`,`dod` FROM `train` WHERE source='$source'&&destn='$destn'";
$a=mysql_query($sql) or die("unable to fetch data");
}
?>
<html>
<head>
<title>Railway Homepage</title>
    <link href="themes/1/js-image-slider.css" rel="stylesheet" type="text/css" />
    <script src="themes/1/js-image-slider.js" type="text/javascript"></script>
    <link href="generic.css" rel="stylesheet" type="text/css" />
    <link href="main.css" rel="stylesheet" type="text/css" />

    <style type="text/css">
<!--
.style1 {
	font-size: 36px;
	color: #FF0000;
	font-weight: bold;
}
.style2 {color: #FF0000}
.style4 {color: #0000FF}
.style5 {font-size: 24px}
.style6 {color: #0000FF; font-size: 24px; }
.style7 {
	font-size: 22px;
	font-weight: bold;
}
-->
    </style>
</head>
<body bgcolor="#009966">
<img class="header" src="train_logo.PNG" width="1200" height="200"><br><br><br>
<ul id="navmenu">
<li><a href="home.html"> HOME</a></li>
<li><a href="te.php">TRAIN ENQUIRY</a></li>
<li><a href="fare.php">FARE ENQUIRY</a></li>
<li><a href="reservation.php">RESERVATION</a></li>
<li><a href="cancel.php">CANCELLATION</a></li>

<li><a href="contactus.html">CONTACT US</a></li>

<br>
<br><br>
<table width="1090"  border="0">
  <tr>
    <th width="212" height="115" scope="col">&nbsp;</th>
    <th width="862" scope="col"><div align="center"><em><span class="style1">Train Enquiry</span></em></div></th>
  </tr>
  <tr>
    <th height="260" scope="row"><p class="style6">Search Train By:-</p>
        <p class="style2"><span class="style4"><span class="style5"><a href="tdate.php" title="date" target="_self">Date</a></span></span></p>
        <p class="style6"><a href="s&d.php" title="s&d" target="_self">Source & Destination</a></p>
            <p class="style2"><span class="style6"><a href="tname.php" title="tname" target="_self">Train Code </a></span></p></th> 
    <td> <p align="center" class="style7">Source &amp; Destination </p>
</div>
<form method="post" action="s&d.php">
<table width="361" border="0" align="center">
  <tr>
    <td><div align="center">Source</div></td>
    <td><div align="center">Destination</div></td>
  </tr>
  <tr>
    <td>
      <label> </label>
      <div align="center">
        <input  type="text" value="Enter Source" name="source"/>
      </div>
    </td>
    <td>
      <label> </label>
      <div align="center">
        <input  type="text" value="Enter Destination"  name="destn"/>
      </div>
   </td>
  </tr>
</table>
<p align="center">&nbsp;</p>
<p align="center">
  <input type="submit" name="Submit" value="Submit"  />
<input type="hidden" name="submitted"  value="true"/>
</p></td>
</form>

<table align="center" border="1">
<tr>
<th><font color='Red'>trainid</font></th>
<th><font color='Red'>train name</font></th>
<th><font color='Red'>dod</font></th>
</tr> 


 
<?php
while($rows = mysql_fetch_array($a,MYSQL_ASSOC))
{ 
$trainid=$rows['trainid'];
$trainname =$rows['trainname'];
$dod=$rows['dod'];
?>
<tr>
 
<td><b><font color='#663300'><?php echo $trainid;?></font></b></td>
<td><b><font color='#663300'><?php echo $trainname;?></font></b></td>
<td><b><font color='#663300'><?php echo $dod;?></font></b></td>

</td>
</tr>
 
<?php } ?>
      <p>&nbsp;</p>
    </div></td>
  </tr>
</table>






</body>
</html>